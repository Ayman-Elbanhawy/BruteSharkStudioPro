    ...
    \ `.         | `.      \|_______|\|__|\|__|\|_______|    \|__|  \|_______|        |\_________\|__|\|__|\|__|\|__|\|__|\|__|\|__| \|__|
     \  `-._     ;   \                                                                \|_________|                                        
      ...
      ,'.-'`,_-._            ((((   <o.   ,'							
        ...
        internal static void CrackHashesWithHashcat(
            string dirPath,
            HashSet<PcapAnalyzer.NetworkHash> hashes,
            string hashcatPath,
            string wordlistPath,
            string extraArguments = null)
        {
            string hashesPath = Path.Combine(dirPath, "Hashes");
            string crackedPath = Path.Combine(dirPath, "Cracked Hashes");
            Directory.CreateDirectory(hashesPath);
            Directory.CreateDirectory(crackedPath);

            foreach (string hashType in hashes.Select(hash => hash.HashType).Distinct())
            {
                try
                {
                    var hashesForType = hashes.Where(h => h.HashType == hashType).ToList();
                    var mode = BruteForce.HashcatRunner.GetHashcatMode(
                        CommonUi.Casting.CastAnalyzerHashToBruteForceHash(hashesForType.First()));
                    var hashFilePath = CommonUi.Exporting.GetUniqueFilePath(Path.Combine(hashesPath, $"Brute Shark - {hashType} Hashcat Export.txt"));
                    var outputFilePath = CommonUi.Exporting.GetUniqueFilePath(Path.Combine(crackedPath, $"Brute Shark - {hashType} Cracked.txt"));

                    using (var streamWriter = new StreamWriter(hashFilePath, true))
                    {
                        foreach (var hash in hashesForType)
                        {
                    ...
                    Console.WriteLine($"Starting Hashcat for {hashType} with mode {mode}");
                    var result = BruteForce.HashcatRunner.CrackHashFile(
                        hashcatPath,
                        mode,
                        hashFilePath,
                        wordlistPath,
                    ...
                    if (!string.IsNullOrWhiteSpace(result.ShowOutput))
                    {
                        ...
                        Console.WriteLine(result.ShowOutput);
                    }
                }
                catch (Exception ex)
                {
                    Console.Error.WriteLine($"Failed to crack {hashType}: {ex.Message}");
                }
            }
        }
