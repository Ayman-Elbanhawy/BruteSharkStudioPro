...
namespace BruteSharkCli
{
    class CliShell
    {
        ...
        public CliShell(PcapAnalyzer.Analyzer analyzer, PcapProcessor.Processor processor,Sniffer sniffer, string seperator = ">")
        {
            ...
            AddCommand(new CliShellCommand("crack-hashes", p => CrackHashes(p), "Crack hashes with Hashcat. Usage: crack-hashes <OUTPUT-DIRECTORY>;<WORDLIST-PATH>;<HASHCAT-PATH optional>"));
        ...
        private void CrackHashes(string parameters)
        {
            var parts = parameters.Split(';');

            if (parts.Length < 2)
            {
                Console.WriteLine("Usage: crack-hashes <OUTPUT-DIRECTORY>;<WORDLIST-PATH>;<HASHCAT-PATH optional>");
                return;
            }

            var outputDirectory = parts[0];
            var wordlistPath = parts[1];
            var hashcatPath = parts.Length > 2 ? parts[2] : null;

            Utilities.CrackHashesWithHashcat(outputDirectory, _hashes, hashcatPath, wordlistPath);
        }
